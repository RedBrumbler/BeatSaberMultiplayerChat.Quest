...
#pragma region Core Events
    void ChatViewController::PostParse() {
        ...
        scrollableContainer->_pageUpButton->onClick->AddListener(custom_types::MakeDelegate<UnityEngine::Events::UnityAction*>(std::function<void()>(
            std::bind(&ChatViewController::HandleScrollablePageUp, this)
        )));
        scrollableContainer->_pageDownButton->onClick->AddListener(custom_types::MakeDelegate<UnityEngine::Events::UnityAction*>(std::function<void()>(
    ...
    void ChatViewController::DidActivate(bool firstActivation, bool addedToHierarchy, bool screenSystemEnabling) {
        if (firstActivation)
            BSML::parse_and_construct(Assets::Views::ChatViewController_bsml, get_transform(), this);
...
#pragma region Messages data/rendering
    ...
    std::pair<TMPro::TextMeshProUGUI*, HMUI::ImageView*> ChatViewController::AddMessageObject() {
        ...
        auto imageRect = imageGo->transform.cast<UnityEngine::RectTransform>();
...
#pragma region Scroll pain
    ...
    void ChatViewController::HandleScrollablePageDown() {
        if (!scrollableContainer) return;
        _chatLockedToBottom = !scrollableContainer->_pageDownButton->interactable;
    }
