...
namespace MultiplayerChat::Core {
    ...
    void ChatManager::Initialize() {
        ...
        _sessionConnectedAction = custom_types::MakeDelegate<std::decay_t<decltype(_sessionConnectedAction)>>(std::function<void()>(
            std::bind(&ChatManager::HandleSessionConnected, this)
        ));
        _sessionDisconnectedAction = custom_types::MakeDelegate<std::decay_t<decltype(_sessionDisconnectedAction)>>(std::function<void(GlobalNamespace::DisconnectedReason)>(
            std::bind(&ChatManager::HandleSessionDisconnected, this, std::placeholders::_1)
        ));
        _sessionPlayerConnectedAction = custom_types::MakeDelegate<std::decay_t<decltype(_sessionPlayerConnectedAction)>>(std::function<void(GlobalNamespace::IConnectedPlayer*)>(
            std::bind(&ChatManager::HandleSessionPlayerConnected, this, std::placeholders::_1)
        ));
        _sessionPlayerDisconnectedAction = custom_types::MakeDelegate<std::decay_t<decltype(_sessionPlayerDisconnectedAction)>>(std::function<void(GlobalNamespace::IConnectedPlayer*)>(
